...
tauSq.samps <- t(read_tsv("chain-1-tauSq", col_names=FALSE))
...
tauSq.true <- as.vector(as.matrix(read.table("sim_data/tauSq")))
...
tauSq.q <- apply(tauSq.samps, 2, quants)
...
q.plot(lambda.true, lambda.q, "lambda")
q.plot(tauSq.true, tauSq.q, "tauSq")
#dev.off()
