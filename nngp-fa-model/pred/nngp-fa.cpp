...
int main(int argc, char **argv){
  ...
  int n0; par.getVal("n0", n0); //number of locations for prediction.
  ...
  int *nnIndx0 = NULL; par.getFile("nnIndx0.file", nnIndx0, i, j);
  
  double *beta = NULL; par.getFile("beta.samples.file", beta, i, j);
  double *tauSq = NULL; par.getFile("tauSq.samples.file", tauSq, i, j);
  double *lambda = NULL; par.getFile("lambda.samples.file", lambda, i, j);
  double *phi = NULL; par.getFile("phi.samples.file", phi, i, j);
  ...
  double *y0 = new double[h*n0*nSamples]; zeros(y0, h*n0*nSamples);
  double *w0 = new double[q*n0*nSamples]; zeros(w0, q*n0*nSamples);
  double *lambdaW0 = new double[h*n0*nSamples]; zeros(lambdaW0, h*n0*nSamples);
  ...
  int zWIndx = -1;
  double *wZ = new double[q*n0*nSamples];
  for(i = 0; i < q*n0*nSamples; i++){
    wZ[i] = rnorm(0.0, 1.0);
  }

  int zYIndx = -1;
  double *yZ = new double[h*n0*nSamples];
  for(i = 0; i < h*n0*nSamples; i++){
    yZ[i] = rnorm(0.0, 1.0);
  }
...
#pragma omp parallel for private(threadID, j, k, l, d, info)
	...
	d = 0;
	for(k = 0; k < m; k++){
	  d += tmp_m[threadID*m+k]*w[s*nq+nnIndx0[i+n0*k]*q+j]; 
	}

        #pragma omp atomic
	zWIndx++;
   
	w0[s*q*n0+i*q+j] = sqrt(1.0 - ddot_(&m, &tmp_m[threadID*m], &inc, &c[threadID*m], &inc))*wZ[zWIndx] + d;

      }//end j
      
      dgemv_(&ntran, &h, &q, &one, &lambda[s*hq], &h, &w0[s*q*n0+i*q], &inc, &zero, &lambdaW0[s*h*n0+i*h], &inc);
	...
	y0[s*h*n0+i*h+j] = sqrt(tauSq[s*h+j])*yZ[zYIndx] + ddot_(&p, &X0[j*n0*p + i*p], &inc, &beta[s*h*p + j*p], &inc) + lambdaW0[s*h*n0+i*h+j];
  ...
  writeRMatrix(outFile+"-w0", w0, q*n0, nSamples);
  writeRMatrix(outFile+"-lambdaW0", lambdaW0, h*n0, nSamples);
  writeRMatrix(outFile+"-z0", y0, h*n0, nSamples);
